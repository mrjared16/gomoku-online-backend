...
export class AuthService {
  ...
  async loginWithGoogleOAuthToken(googleOAuthToken: string) {
    ...
    if (googleUserData == null) {
      throw new HttpException('Error when verify Google OAuth', HttpStatus.SERVICE_UNAVAILABLE);
    }
    const { email, name } = googleUserData;
    const userWithThisUsername = await this.userService.findUser({ username: email });
    if (userWithThisUsername) {
      return this.getToken(userWithThisUsername);
    }

    const newUser = await this.userService.createUser({ name, username: email, password: null });
